...
it('has exactly one build, one deploy and one rollback implementation', function () {
    ...
    expect($actions)->toBe([
        ...
        // the controlled code alignment's one addition: one RESTORE implementation, covering all
...
it('keeps one operator-facing workflow per environment, with no target selector anywhere', function () {
    expect(array_keys(phase71Workflows()))->toEqualCanonicalizing([
        ...
        'release.yml',
        // the controlled code alignment: one restore workflow per environment, exactly like every
...
it('implements nothing from Repair Target onwards', function () {
    ...
    // its own scope guard (DeploymentObservabilityScopeTest, RestoreServerPrimitivesScopeTest,
    // RestoreOperatorSurfaceScopeTest). Repair and Recover remain future work, and nothing
