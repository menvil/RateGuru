    ...
    @if ($paginator->hasPages())
        @php
            ...
            if ($last <= 7) {
                ...
                // Fill single-page gaps to avoid lone ellipsis ("3 … 5" → "3 4 5")
                $filled = collect();
                $arr = $set->toArray();
        ...
        <nav role="navigation" aria-label="Pagination Navigation"
            ...
            @foreach ($pages as $page)
                @if ($prev !== null && $page - $prev > 1)
                    <span class="rg-pgn-dot" aria-hidden="true">…</span>
                @endif

                @if ($page === $current)
                    <span class="rg-pgn-btn rg-pgn-active" aria-current="page"
                          wire:key="paginator-{{ $name }}-page{{ $page }}">{{ $page }}</span>
                @else
                    <button type="button"
                            wire:click="gotoPage({{ $page }}, '{{ $name }}')"
                            @if($scrollIntoViewJsSnippet) x-on:click="{{ $scrollIntoViewJsSnippet }}" @endif
                            class="rg-pgn-btn"
                            wire:key="paginator-{{ $name }}-page{{ $page }}"
